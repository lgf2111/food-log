...
describe('GET /api/search', () => {
  it('finds meals by food name (case-insensitive, owner-scoped)', async () => {
    const tgId = 5001;
    await save(tgId, 'Chicken Rice');
    await save(tgId, 'beef noodles');
    const app = createApp();

    const res = await app.request('/api/search?q=chicken', { headers: await headers(tgId) }, env);
    expect(res.status).toBe(200);
    const body = (await res.json()) as { meals: Array<{ foods: string[] }> };
    expect(body.meals).toHaveLength(1);
    expect(body.meals[0]?.foods).toContain('Chicken Rice');
  });

  it('returns empty for an empty query', async () => {
    const app = createApp();
    const res = await app.request('/api/search?q=', { headers: await headers(5002) }, env);
    const body = (await res.json()) as { meals: unknown[] };
    expect(body.meals).toEqual([]);
  });

  it('does not return other users\' matches', async () => {
    await save(5003, 'unique-tofu-dish');
    const app = createApp();
    const res = await app.request('/api/search?q=tofu', { headers: await headers(5004) }, env);
    const body = (await res.json()) as { meals: unknown[] };
    expect(body.meals).toEqual([]);
  });
  ...
  it('treats LIKE wildcards in the query as literal text', async () => {
    const tgId = 5005;
    await save(tgId, 'plain oats');
    const app = createApp();
    // "%" should not act as a wildcard matching everything.
    const res = await app.request('/api/search?q=%25', { headers: await headers(tgId) }, env);
    const body = (await res.json()) as { meals: unknown[] };
    expect(body.meals).toEqual([]);
  });
});
