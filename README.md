...
The Mini App (shadcn/ui + Tailwind v4, Telegram theming, native buttons + haptics) is now a focused two-tab app: **Home** and **Settings**. Home is a day view — a clickable calendar to pick any day, that day's totals, and its meals; each meal supports **swipe-left-to-delete** and **"Update with AI"** (describe a change in plain words — "add a can of coke", "the rice was double" — and the AI rewrites the log via `POST /api/meals/:id/revise`). The History, Search, and Stats tabs (and their endpoints) were removed. Screens are code-split (`React.lazy`) and Home fetches only the selected day (`GET /api/meals?date=` + `GET /api/meals/dates`) for a fast first load. See `PLAN.md` §10–§11 for the full progress log and roadmap.
